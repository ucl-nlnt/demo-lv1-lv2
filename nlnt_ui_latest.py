...
def nlnt (prompt,  history="None", progress=gr.Progress()):
...
def level2_model(user_instruction, progress=gr.Progress()):
    ...
    global starting_position, starting_orientation
    global total_distance, total_rotation, normalizing_quat, deconstruction
    global stop_flag
    ...
    while True:
        ...
        if not possibility:
            ...
            deconstruction = prompt_breakdown
        ...
        while state_number <= max_num_states:
            ...
            if max_num_states != 0:
                progress(state_number/max_num_states, desc="Ongoing..")
            else:
                progress(0, desc="Ongoing..")
            ...
            total_distance = received['total distance']
            total_rotation = round(received['total angular distance'] * 180 / math.pi, 3)
            print(received['total distance'], received['distance_traveled'])
            print(user_instruction)
            todo['state number'] = state_number

            if state_number >= max_num_states or received['blocked'] or stop_flag:
    
                if received['blocked']:
                    progress(1, desc="Blocked by obstacle!")
                elif stop_flag:
                    progress(1, desc="Forced stop by user.")
                else:
...
def read_metadata():
    ...
    # print("global", total_distance, total_rotation, deconstruction)
...
with gr.Blocks(theme=theme, css=css, title = "NLNT Demo",js="metadata.js", head=head) as demo:
    ...
    with gr.Row():
        with gr.Column():
            ...
            #history = None
            run_nlnt = ttbt_btn.click(fn=nlnt, inputs=[ prompt], outputs=status)
            stop = stop_btn.click(fn=stop_nlnt, inputs=[], outputs=status)
    #with gr.Row():
    ...
    with gr.Row():
        ...
        <div style='display: flex; flex-direction: column; height: auto; width: 100%;'>
            <div style='display: flex; flex-direction: column; padding: 20px; background-color: #292524;'>
                ...
                <span id="prompt" style='background-color: #44403C; padding: 20px;margin-top: 20px;white-space:pre-wrap;'>0</span>
            </div>
        </div>
        <br>
        <br>
